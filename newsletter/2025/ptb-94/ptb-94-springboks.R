...
table_overall <- read_sheet(ss = link, sheet = "Table") %>%
  arrange(desc(Rating)) %>%
  select(-Rank)

display_tbl_overall <- table_overall %>%
  gt(rowname_col = "Team") %>%
...
tbl_formatted_overall <- display_tbl_overall %>%
  tab_header(title = html("Despite some mixed results, the <b style='color:#166642'>South Africa</b> remain men's rugby's best team"),
             subtitle = html("<b>'Team rating'</b> of each Tier 1 nation in men's rugby union, expressed in points per game <b>better or worse than average</b><br><br>")) %>%
  tab_source_note(source_note = html("<br>'Rating' reflects a team's four most recent results against each other nation, adjusted for home advantage; more weight is given to more recent games<br><br><br>Data: Plot the Ball | Table: Plot the Ball<br>")) %>%
  tab_stubhead(label = "Team") %>%
  tab_spanner(label = md("— ADJUSTED MARGIN VS. —"),
              ### MANUALLY RE-ORDER COLUMNS TO MATCH RANKING
              columns = c(ITA, WLS, AUS, ARG, SCT, ENG, FRA, IRL, NZL, ZAF)) %>%
  ...
  cols_label(Rating = "Rating",
             ...
             ZAF = "ZA",
             WLS = "WA") %>%
  tab_style(locations = cells_stub(rows = "South Africa"),
            style =  list(cell_text(align = "left",
                                    weight = "bold",
                                    color = "#166642",
                                    size = "medium"),
                          cell_fill(color = "#16664220"))) %>%
  tab_style(locations = cells_stub(rows = c("Australia", "Argentina", "New Zealand")),
            style =  list(cell_text(align = "left",
                                    color = ptb_dark_grey_v2,
                                    weight = "bold",
                                    size = "medium"),
                          cell_fill(color = ptb_light_grey_v4))) %>%
  tab_style(locations = cells_body(rows = c("Ireland", "England", "Scotland", "France", "Wales", "Italy")),
            style =  list(cell_text(color = ptb_mid_grey))) %>%
  tab_style(locations = cells_body(rows = c("Australia", "Argentina", "New Zealand")),
            style =  list(cell_fill(color = ptb_light_grey_v4))) %>%
  tab_style(locations = cells_body(rows = "South Africa"),
            style =  list(cell_fill(color = "#16664220"))) %>%
  data_color(columns = Rating,
             rows = c("Australia", "South Africa", "Argentina", "New Zealand"),
