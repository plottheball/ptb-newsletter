...
top_url_base <- "https://stats.espncricinfo.com/ci/engine/stats/index.html?batting_positionmax1=3;batting_positionmin1=1;batting_positionval1=batting_position;class=9;filter=advanced;orderby=start;page="
first_page <- 1
...
top_player_match_splits <- top_all_inns_clean %>%
  group_by(match_id, name, country, year) %>%
  summarise(n_inns = n(),
            n_runs = sum(runs),
            n_balls = sum(bf),
...
final_table <- top_vs_exp_summary %>%
  filter(name == "L Wolvaardt") %>%
  select(year, n_inns, ave, other_ave, ave_vs_exp, sr, other_sr, sr_vs_exp) %>%
  arrange(year)

display_tbl <- final_table %>%
  gt(rowname_col = "year",
     groupname_col = NA) %>%
  cols_hide(columns = c("name", "country")) %>%
...
positive_colour <- "#1F6640"
negative_colour <- "#CCA63D"
positive_fill <- str_c(positive_colour, "10")
negative_fill <- str_c(negative_colour, "10")

tbl_formatted <- display_tbl %>%
  tab_header(title = html("At the start of her ODI career, Wolvaardt's scoring rate was <b style='color:#CCA63D'>lower</b> than other batters'"),
             subtitle = html("Career performance of <b>Laura Wolvaardt</b> in women's ODIs, compared to the record of <b>other top-order batters in her games</b><br><br>")) %>%
  tab_source_note(source_note = html("<br>'Other' = performance of other top-order batters (i.e. positions 1-3) in a player's games<br><br>'Average' = runs scored per dismissal | 'Strike rate' = runs scored per 100 balls<br><br>Data: ESPNCricinfo | Table: Plot the Ball<br>")) %>%
  ...
  tab_style(locations = cells_body(columns = c(ave, sr)),
            style = list(cell_borders(sides = c("left"),
                                      weight = px(2),
                                      color = ptb_light_grey))) %>%
  tab_style(locations = cells_body(columns = c(ave_vs_exp),
                                   rows = ave_vs_exp > 0),
            style = list(cell_text(color = positive_colour),
                         cell_fill(color = positive_fill))) %>%
  tab_style(locations = cells_body(columns = c(ave_vs_exp),
                                   rows = ave_vs_exp < 0),
            style = list(cell_text(color = negative_colour),
                         cell_fill(color = negative_fill))) %>%
  tab_style(locations = cells_body(columns = c(sr_vs_exp),
                                   rows = sr_vs_exp > 0),
            style = list(cell_text(color = positive_colour),
                         cell_fill(color = positive_fill))) %>%
  tab_style(locations = cells_body(columns = c(sr_vs_exp),
