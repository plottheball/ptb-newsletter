...
table_defence <- read_sheet(ss = link, sheet = "Table - defence") %>%
  arrange(desc(Rating)) %>%
  select(-Rank)

display_tbl_defence <- table_defence %>%
  gt(rowname_col = "Team") %>%
...
tbl_formatted_defence <- display_tbl_defence %>%
  tab_header(title = html("<b style='color:#D98921'>Australia</b> concede more points than the Lions' other traditional opponents"),
             subtitle = html("<b>'Defence rating'</b> of each Tier 1 nation in men's rugby union, expressed in points per game <b>better or worse than average</b><br><br>")) %>%
  tab_source_note(source_note = html("<br><i>Data as at 14 Jul 2025</i><br><br>'Defence' reflects a team's four most recent results against each other nation, adjusted for home advantage; more weight is given to more recent games<br><br><br>Data: Plot the Ball | Table: Plot the Ball<br>")) %>%
  tab_stubhead(label = "Team") %>%
  tab_spanner(label = md("— ADJUSTED POINTS CONCEDED VS. —"),
              ### MANUALLY RE-ORDER COLUMNS TO MATCH RANKING
              columns = c(ITA, WLS, ARG, AUS, FRA, SCT, ENG, NZL, IRL, ZAF)) %>%
  ...
  cols_label(Rating = "Defence",
             ...
             ZAF = "ZA",
             WLS = "WA") %>%
  tab_style(locations = cells_stub(rows = "Australia"),
            style =  list(cell_text(align = "left",
                                    weight = "bold",
                                    color = "#D98921",
                                    size = "medium"),
                          cell_fill(color = "#D9892120"))) %>%
  tab_style(locations = cells_stub(rows = c("South Africa", "New Zealand")),
            style =  list(cell_text(align = "left",
                                    weight = "bold",
                                    color = ptb_dark_grey,
                                    size = "medium"),
                          cell_fill(color = ptb_light_grey_v4))) %>%
  tab_style(locations = cells_body(rows = c("Ireland", "England", "Scotland", "France", "Argentina", "Wales", "Italy")),
            style =  list(cell_text(color = ptb_mid_grey))) %>%
  tab_style(locations = cells_body(rows = c("South Africa", "New Zealand")),
            style =  list(cell_fill(color = ptb_light_grey_v4))) %>%
  tab_style(locations = cells_body(rows = "Australia"),
            style =  list(cell_fill(color = "#D9892120"))) %>%
  data_color(columns = Rating,
             rows = c("Australia", "South Africa", "New Zealand"),
  ...
  tab_style(locations = cells_body(rows = "New Zealand",
                                   columns = "NZL"),
            style =  list(cell_fill(color = fill_other))) %>%
  tab_style(locations = cells_body(rows = "South Africa",
                                   columns = "ZAF"),
            style =  list(cell_fill(color = fill_other))) %>%
  ...
  tab_style(locations = cells_body(rows = "Italy",
                                   columns = "ITA"),
            style =  list(cell_fill(color = fill_other))) %>%
  tab_style(locations = cells_body(rows = "Argentina",
                                   columns = "ARG"),
            style =  list(cell_fill(color = fill_other)))
tbl_formatted_defence
