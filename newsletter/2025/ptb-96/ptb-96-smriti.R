...
active_players_top <- top_all_inns_clean %>%
  filter(start_date > dmy("31-12-2024")) %>%
  select(name) %>%
  unique() %>%
  unlist()
...
names_fix <- tibble(name = top_vs_exp_active$name,
                    batter = c("Suzie Bates", "Tammy Beaumont", "Chamari Athapaththu", "Laura Wolvaardt", "Smriti Mandhana", "Hayley Matthews", "Heather Knight", "Alyssa Healy", "Sidra Amin"))
...
display_tbl <- final_table %>%
  gt(rowname_col = "batter",
     groupname_col = NA) %>%
  cols_hide(columns = c("name", "n_inns")) %>%
...
tbl_formatted <- display_tbl %>%
  tab_header(title = html("<b style='color:#5B85AA'>Smriti Mandhana</b> scores her ODI runs much faster than other top-three batters"),
             subtitle = html("Career performance of <b>active top-order batters</b> in women's ODIs, compared to the record of <b>other top-order batters in their games</b><br><br>")) %>%
  ...
  tab_style(locations = cells_body(columns = c(ave, sr)),
            style = list(cell_borders(sides = c("left"),
                                      weight = px(2),
                                      color = ptb_light_grey))) %>%
  tab_style(locations = cells_body(rows = batter == "Smriti Mandhana"),
            style = list(cell_text(color = ind_colour,
                                   weight = "bold"),
                         cell_fill(color = highlight_ind))) %>%
  tab_style(locations = cells_stub(rows = batter == "Smriti Mandhana"),
            style = list(cell_text(color = ind_colour,
                                   weight = "bold"),
                         cell_fill(color = highlight_ind)))
tbl_formatted
